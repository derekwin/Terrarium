...
|---|---|---|
| **Platform / host admin** | `terra` CLI | Runs `terra daemon` (or a systemd unit), prepares the environment once: `terra setup alpine|ubuntu` (kernel, rootfs, initramfs, base layer with sandlock, template), optional tool layers via `terra tool create`, creates the warm pool |
...
sb1.kill()  # kills this session (running execs + workdir), VM survives
