...
| 4 | `sandlock-v0.8.5-procs.patch` | Fix `--max-processes` bypass via bare fork(2): the supervisor only intercepted clone/clone3/vfork, but busybox ash forks background jobs with fork(2), which was deliberately left out of the BPF notif filter for fork-loop performance. A process limit is meaningless if fork(2) bypasses it, so fork(2) now joins the notif set whenever `max_processes != 64` (the unset sentinel) — the existing `handle_fork` enforces the count and returns EAGAIN. No behavior change when no limit is set. | Not upstreamed; Terrarium-specific correctness fix. | Low: 2 files, ~20 lines (feature flag + notif-list condition). | 2026-08-06 |
## Rebuild Procedure
