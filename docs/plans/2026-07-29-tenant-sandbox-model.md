...
| S-M3 全面对齐 | 网络策略 ✅ 2026-07-30：policy 透传落地（协议/引擎/vsock/SDK 同一 `policy` 对象——`read_paths`/`write_paths` 追加授予、`net_allow` 出站默认拒绝（空列表三层硬报错）、`memory_mb`/`procs` 资源限制；`sandbox_create` 存放 + `sandbox_exec` 单次覆盖 + info/ls 回显；CLI/SDK 旗标齐全）。注意：`net_allow` 出站 ACL 尚未做真机网络验证（需 root daemon 的 NAT 环境；已有 skip-guarded 用例 `test_net_allow_live_egress`）。**剩余**：MCP sandbox 工具 + 池化租户 VM | `tenant_destroy` 全回收，e2e 通过 |
