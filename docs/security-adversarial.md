...
- **预置快照（生产形态）稳态 ~139-539 VMs/s**（`--snapshot-path` 复用
  一份快照，跳过一次性基 VM 引导）：端到端 harness（64 路 + 逐调用连接）
  实测 ~139/s；受控 96 路 restore-only 实测 539/s（见下）。快照是固定
  资产（常驻页缓存），部署后实际速率取决于调用方连接复用程度。
