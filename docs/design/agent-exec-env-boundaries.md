...
E 的目标是**契约可替换性**——engine 零改动、后端可插——而非绑定某个具体后端。

**已实证(层 1:契约与注入路径)**:
1. `engine` 零后端实现:只依赖 `adapter-traits`(trait),不编译任何具体后端
2. `VmManager::with_sandbox_adapter` 注入任意后端;`MockSandboxAdapter` 注入测试
   证明 engine 可接入异构实现而零改动
3. `SandboxAdapter::create(vm, spec)` 不假设后端使用 VM——guest sandlock 经
   `VmHandle::exec`,独立后端(如 host 侧沙箱)可忽略 vm 参数

**受部署环境限制(层 2:运行时验证)**:
- 真实独立后端的运行时验证需能创建命名空间的部署环境
- 本开发环境 mount namespace 被禁(系统级限制),gVisor 等需 mount ns 的后端
  无法建沙箱——这是**部署限制,非契约缺陷**
- 运行时实证延后到具备相应能力的部署环境;契约完备性已由测试与设计证明
...
5. **可替换**:异构实现接入无需改引擎核心(engine 零后端依赖 + 注入测试已实证)。

---

*B/C/D/E 已完成:策略模型、L2 统一、审计可观测、可替换性实证(契约层)全部落地。
运行时第二后端验证受部署环境限制,延后至具备 mount ns 能力的部署。*
